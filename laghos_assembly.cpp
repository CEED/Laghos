...
namespace mfem
{
  ...
  namespace hydrodynamics
  {
    ...
    void NitscheEnergyBoundaryForceIntegrator::AssembleRHSElementVect(const FiniteElement &el,
								      const FiniteElement &el2,
								      FaceElementTransformations &Tr,
								      Vector &elvect)    
    {
	...
	if (Tr.Attribute == 33){
	  ...
	  for (int q = 0; q < nqp_face; q++)
		...
		nor_norm += nor(s) * nor(s);
	      ...
	      nor_norm = sqrt(nor_norm);
	      Vector vShape;
	      Vnpt_gf->GetVectorValue(elementNo, eip_el1, vShape);
	      double vDotn = 0.0;
	      for (int s = 0; s < dim; s++)
	      {
		vDotn += vShape(s) * nor(s)/nor_norm;
		...
		for (int i = 0; i < l2dofs_cnt; i++)
		  {
		    elvect(i) += weighted_1 * vDotn * shape_el1(i) * pressure_diff * ip_f.weight * nor_norm;
		    elvect(i + l2dofs_cnt) += weighted_1 * vDotn * shape_el2(i) * pressure_diff * ip_f.weight * nor_norm;
		  }
	      }
	    }
	}
