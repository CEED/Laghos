...
RemapAdvector::RemapAdvector(const ParMesh &m, int order_v, int order_e,
    ...
    v_ess_tdofs(ess_tdofs), v_ess_vdofs(ess_vdofs),
    remap_scheme(remap_),
    remap_v(remap_v_),
    cfl_factor(cfl),
    visualize(vis),
                                 ...
                                 const ParGridFunction &eps)
{
   ...
   *x = x0;
   ...
   // Velocity
   SolutionTransfer_H1 transfer_h1(v_ess_tdofs, v_ess_vdofs, pfes_H1, pfes_H1_s, rho_ir);
   switch (remap_scheme)
   {
   case RemapScheme::Nonconservative:
      if (remap_v != RemapVelocity::HighOrder)
   ...
   *x = x0;
   ...
   case RemapScheme::Nonconservative:
   {
      oper = new AdvectorNonconservativeOper(
         x0, v_ess_tdofs, v_ess_vdofs, u, Sgf[Density], *ir_rho,
   ...
   if (u_max == 0.0)
   {
      const real_t mass = oper->Mass(Sgf[Density], 0.);
      const real_t momentum = oper->Momentum(Sgf[Velocity], 0.);
      const real_t energy = oper->InternalEnergy(Sgf[Energy], 0.);
      ...
      return; // No need to change the fields.
   }
                                   ...
                                   ParGridFunction &eps)
{
   ...
   // Velocity
   SolutionTransfer_H1 transfer_h1(v_ess_tdofs, v_ess_vdofs, pfes_H1, pfes_H1_s, ir_rho);
   switch (remap_scheme)
   {
   case RemapScheme::Nonconservative:
      if (remap_v != RemapVelocity::HighOrder)
...
: AdvectorVelocityOper(v_ess_td, v_ess_vd, pfes_H1, pfes_H1_s, scheme),
  ...
  detJ(&pfes_H1_s), rhoJ(&pfes_H1_s), KJ(pfes_H1_s.GetVSize())
{
   // Solution transfer
...
void AdvectorVelocityGeomConsOper::MultConserv(const ParGridFunction &flux, const Vector &U, Vector &K_v, Vector &dU) const
{
   ...
   // Current density * Jacobian (needed in LimitUpdate())
...
void AdvectorVelocityGeomConsOper::LimitUpdate(real_t dt, const Vector &U, const Vector &K_v, Vector &dU)
{
   ...
   const int vdim = pfes_H1.GetVDim();
   const int ndof = pfes_H1.GetNDofs();
   for (int v = 0; v < vdim; v++)
   {
      SparseMatrix M_sm;
      ...
      const Vector U_v(const_cast<Vector&>(U), v*ndof, ndof);
      Vector dU_v(dU, v*ndof, ndof);

      switch (remap_v)
      {
      case RemapAdvector::RemapVelocity::MCL:
         MCLProduct(K_sm, KT_sm, M_sm, U_v, rhoJ, dU_v);
         break;
      default:
   ...
   const Vector &xy, const Vector &x, Vector &d_xy) const
{
   ...
   HypreParVector m_hpr(&pfes_H1_s), xy_hpr(&pfes_H1_s), x_hpr(&pfes_H1_s), /*xydot(&pfes_H1_s),*/ d_xy_hpr(&pfes_H1_s), y_min(&pfes_H1_s), y_max(&pfes_H1_s);
   ...
   for(int i = 0; i < dofs_h1; i++)
   {
      ...
      for(int k = I[i_td]; k < I[i_td+1]; k++)
      {
         ...
         const real_t x_i = x_glb->Elem(i_gl);
         const real_t x_j = m_glb->Elem(j_gl) != 0. ? x_glb->Elem(j_gl) : 0.;
         const real_t xy_i = xy_glb->Elem(i_gl);
         const real_t xy_j = m_glb->Elem(j_gl) != 0. ? xy_glb->Elem(j_gl) : 0.;

         const real_t dij = max(max(0.0,-kji),-kij);
         //const real_t dij = max( abs(kij), abs(kji));
         real_t fij = M[k] * (d_xy_glb->Elem(i_gl) - d_xy_glb->Elem(j_gl)) + dij * (xy_i - xy_j);

         //limit target flux to enforce local bounds for the bar states (note, that dij = dji)
         const real_t wxij = dij * (x_i + x_j) + kij * (x_j - x_i);
         const real_t wxji = dij * (x_i + x_j) + kji * (x_i - x_j);
         const real_t wxyij = dij * (xy_i + xy_j) + kij * (xy_j - xy_i);
         const real_t wxyji = dij * (xy_i + xy_j) + kji * (xy_i - xy_j);

         if(fij > 0)
         {
            const real_t fij_bound = min(wxij * ymax_glb->Elem(i_gl) - wxyij,
                                         wxyji - wxji * ymin_glb->Elem(j_gl));
            fij = min(fij, fij_bound);

            // to get rid of rounding errors wich influence the sign
            //fij = max(0.0, fij_star);
         }
         else
         {
            const real_t fij_bound = max(wxij * ymin_glb->Elem(i_gl) - wxyij,
                                         wxyji - wxji * ymax_glb->Elem(j_gl));
            fij = max(fij, fij_bound);

            // to get rid of rounding errors wich influence the sign
            //fij_star = min(0.0, fij_star);
         }

         rhs_array[i] += (dij + kij) * xy_j - (dij + kji) * xy_i + fij;
      }
   }
