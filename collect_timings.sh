...
echo "# order refs h1_dofs l2_dofs h1_cg_rate l2_cg_rate forces_rate update_quad_rate" > $outfile
for torder in "${thermo_orders[@]}"; do
    for sref in "${serial_refs[@]}"; do
       echo "np"$nproc "Q"$((torder+1))"Q"$torder $sref"ref"
       echo $(run_case mpirun -np $nproc ./laghos -pa -p 1 -tf 0.8 -cfl 0.05 \
                       --max_steps 50 \
                       --mesh $mesh_file2D \
                       --refine-serial $sref \
                       --refine-parallel $parallel_refs \
                       --order-thermo $torder \
                       --order-kinematic $((torder+1))) >> $outfile
    done
done
