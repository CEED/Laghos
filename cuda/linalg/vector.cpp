...
CudaVector::CudaVector(const CudaVector *v):size(v->size),data(v->data),own(false){}
  
CudaVector::CudaVector(CudaArray<double>& v):size(v.size()),data(v.ptr()),own(false){}

// Host 2 Device ***************************************************************
CudaVector::CudaVector(const Vector& v):size(v.Size()),data(alloc(size)),own(true){
  assert(v.GetData());
